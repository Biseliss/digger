...
public class SmokeTest {
    ...
    public static void main(String[] a) {
        ...
        // 13a. копка СЕРЕДИНЫ лестницы: огрызок у пола (подкреплён снизу) остаётся
        // стоять, а оторванный от опоры кусок сверху обрушивается сам — но всё
        // равно возвращается в карман вместе с выкопанным тайлом, а не пропадает
        // (раньше обрушенные тайлы терялись — с точки зрения игрока выглядело
        // так, будто лестница "перестала выпадать")
        int lx2 = 30;
        ...
        // выкопанный тайл (1) + оторванный от опоры кусок сверху (8, до самого верха колонны)
        check("копка середины лестницы возвращает выкопанный тайл и обрушившийся кусок (" + brokenMid.size() + " шт)",
                brokenMid.size() == 9 && miner.getUtility(UtilityType.LADDER) == had + 9);
        check("огрызок лестницы у пола остался — он подкреплён снизу",
                f.isLadder(lx2, floorY2 - 1));
        check("оторванный от опоры кусок лестницы обрушился и пропал из мира",
